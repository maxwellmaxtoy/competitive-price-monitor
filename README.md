...
Every monitor execution creates a record in the SQLite `run\_history` table.
...
The included `monitor\_health.py` script provides operational information such as:
...
.venv\\Scripts\\python.exe monitor\_health.py
...
setx TELEGRAM\_BOT\_TOKEN "YOUR\_BOT\_TOKEN"

setx TELEGRAM\_CHAT\_ID "YOUR\_CHAT\_ID"
...
.venv\\Scripts\\python.exe price\_monitor.py
...
.venv\\Scripts\\python.exe test\_partial\_failure.py
...
\- The run is classified as `PARTIAL\_FAILURE`
...
.venv\\Scripts\\python.exe test\_telegram\_suppression.py
